...
function migrateStateIfNeeded(state) {
  ...
  if (!Number.isFinite(Number(next.ouncesAvailableForSale))) {
    next.ouncesAvailableForSale =
      (C.BRICKS_AVAILABLE_FOR_SALE || 0) * (C.OUNCES_PER_BRICK || 36);
  }
  ...
  // ✅ New fields
  if (typeof next.earlyBirdBadge !== "boolean") next.earlyBirdBadge = true;
  if (typeof next.buyPaused !== "boolean") next.buyPaused = false;

  if (!Array.isArray(next.rewardRounds)) next.rewardRounds = [];
  if (!next.claimedRewardsStable || typeof next.claimedRewardsStable !== "object") {
    next.claimedRewardsStable = {};
  }
